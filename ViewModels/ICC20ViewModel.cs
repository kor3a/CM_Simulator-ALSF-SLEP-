...
public partial class ICC20ViewModel : ViewModelBase
{
    ...
    public ICC20ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC20: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC20 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC20: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC20: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC20: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC20 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC20: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC20: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC20 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC20: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC20: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC20 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc20)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC20: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC20: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC20 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC20: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc20On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc20On = false;
            _mainViewModel.Icc20SideMenu = "OFF";
            _mainViewModel.Icc20BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc20BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc20PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc20Rem = false;

            _mainViewModel.icc20MessageData = (byte)(_mainViewModel.icc20MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc20MessageData = (byte)(_mainViewModel.icc20MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc20Low)
            {
                _mainViewModel.icc20Low = false;
                _mainViewModel.icc20MessageData = (byte)(_mainViewModel.icc20MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc20Med)
            {
                _mainViewModel.icc20MessageData = (byte)(_mainViewModel.icc20MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc20Med = false;
            }
            if (_mainViewModel.icc20High)
            {
                _mainViewModel.icc20MessageData = (byte)(_mainViewModel.icc20MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc20High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc20On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc20On = true;
            _mainViewModel.Icc20SideMenu = "";
            _mainViewModel.Icc20BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc20BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc20PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc20On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc20Rem)
            {
                _mainViewModel.icc20Rem = true;
    ...
    {
        if (_mainViewModel.icc20On && !_mainViewModel.icc20Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc20Med)
            {
                _mainViewModel.icc20MessageData = (byte)(_mainViewModel.icc20MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc20Low = true;
            _mainViewModel.Icc20SideMenu = "LOW";
            _mainViewModel.Icc20BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc20BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc20PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc20On && !_mainViewModel.icc20Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc20Low)
            {
                _mainViewModel.icc20MessageData = (byte)(_mainViewModel.icc20MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc20Med = true;
            _mainViewModel.Icc20SideMenu = "MED";
            _mainViewModel.Icc20BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc20BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc20PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc20On && !_mainViewModel.icc20High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc20Low)
            {
                _mainViewModel.icc20MessageData = (byte)(_mainViewModel.icc20MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc20High = true;
            _mainViewModel.Icc20SideMenu = "HIGH";
            _mainViewModel.Icc20BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc20BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc20PgBackground = new SolidColorBrush(Colors.OrangeRed);
