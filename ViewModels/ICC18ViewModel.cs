...
public partial class ICC18ViewModel : ViewModelBase
{
    ...
    public ICC18ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC18: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC18 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC18: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC18: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC18: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC18 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC18: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC18: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC18 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC18: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC18: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC18 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc18)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC18: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC18: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC18 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC18: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc18On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc18On = false;
            _mainViewModel.Icc18SideMenu = "OFF";
            _mainViewModel.Icc18BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc18BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc18PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc18Rem = false;

            _mainViewModel.icc18MessageData = (byte)(_mainViewModel.icc18MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc18MessageData = (byte)(_mainViewModel.icc18MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc18Low)
            {
                _mainViewModel.icc18Low = false;
                _mainViewModel.icc18MessageData = (byte)(_mainViewModel.icc18MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc18Med)
            {
                _mainViewModel.icc18MessageData = (byte)(_mainViewModel.icc18MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc18Med = false;
            }
            if (_mainViewModel.icc18High)
            {
                _mainViewModel.icc18MessageData = (byte)(_mainViewModel.icc18MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc18High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc18On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc18On = true;
            _mainViewModel.Icc18SideMenu = "";
            _mainViewModel.Icc18BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc18BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc18PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc18On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc18Rem)
            {
                _mainViewModel.icc18Rem = true;
    ...
    {
        if (_mainViewModel.icc18On && !_mainViewModel.icc18Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc18Med)
            {
                _mainViewModel.icc18MessageData = (byte)(_mainViewModel.icc18MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc18Low = true;
            _mainViewModel.Icc18SideMenu = "LOW";
            _mainViewModel.Icc18BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc18BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc18PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc18On && !_mainViewModel.icc18Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc18Low)
            {
                _mainViewModel.icc18MessageData = (byte)(_mainViewModel.icc18MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc18Med = true;
            _mainViewModel.Icc18SideMenu = "MED";
            _mainViewModel.Icc18BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc18BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc18PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc18On && !_mainViewModel.icc18High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc18Low)
            {
                _mainViewModel.icc18MessageData = (byte)(_mainViewModel.icc18MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc18High = true;
            _mainViewModel.Icc18SideMenu = "HIGH";
            _mainViewModel.Icc18BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc18BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc18PgBackground = new SolidColorBrush(Colors.OrangeRed);
