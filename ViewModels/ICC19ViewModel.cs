...
public partial class ICC19ViewModel : ViewModelBase
{
    ...
    public ICC19ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC19: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC19 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC19: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC19: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC19: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC19 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC19: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC19: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC19 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC19: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC19: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC19 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc19)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC19: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC19: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC19 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC19: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc19On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc19On = false;
            _mainViewModel.Icc19SideMenu = "OFF";
            _mainViewModel.Icc19BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc19BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc19PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc19Rem = false;

            _mainViewModel.icc19MessageData = (byte)(_mainViewModel.icc19MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc19MessageData = (byte)(_mainViewModel.icc19MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc19Low)
            {
                _mainViewModel.icc19Low = false;
                _mainViewModel.icc19MessageData = (byte)(_mainViewModel.icc19MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc19Med)
            {
                _mainViewModel.icc19MessageData = (byte)(_mainViewModel.icc19MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc19Med = false;
            }
            if (_mainViewModel.icc19High)
            {
                _mainViewModel.icc19MessageData = (byte)(_mainViewModel.icc19MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc19High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc19On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc19On = true;
            _mainViewModel.Icc19SideMenu = "";
            _mainViewModel.Icc19BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc19BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc19PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc19On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc19Rem)
            {
                _mainViewModel.icc19Rem = true;
    ...
    {
        if (_mainViewModel.icc19On && !_mainViewModel.icc19Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc19Med)
            {
                _mainViewModel.icc19MessageData = (byte)(_mainViewModel.icc19MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc19Low = true;
            _mainViewModel.Icc19SideMenu = "LOW";
            _mainViewModel.Icc19BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc19BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc19PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc19On && !_mainViewModel.icc19Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc19Low)
            {
                _mainViewModel.icc19MessageData = (byte)(_mainViewModel.icc19MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc19Med = true;
            _mainViewModel.Icc19SideMenu = "MED";
            _mainViewModel.Icc19BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc19BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc19PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc19On && !_mainViewModel.icc19High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc19Low)
            {
                _mainViewModel.icc19MessageData = (byte)(_mainViewModel.icc19MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc19High = true;
            _mainViewModel.Icc19SideMenu = "HIGH";
            _mainViewModel.Icc19BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc19BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc19PgBackground = new SolidColorBrush(Colors.OrangeRed);
