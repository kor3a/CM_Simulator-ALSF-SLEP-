...
public partial class ICC15ViewModel : ViewModelBase
{
    ...
    public ICC15ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC15: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC15 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC15: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC15: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC15: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC15 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC15: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC15: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC15 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC15: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC15: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC15 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc15)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC15: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC15: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC15 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC15: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc15On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc15On = false;
            _mainViewModel.Icc15SideMenu = "OFF";
            _mainViewModel.Icc15BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc15BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc15PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc15Rem = false;

            _mainViewModel.icc15MessageData = (byte)(_mainViewModel.icc15MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc15MessageData = (byte)(_mainViewModel.icc15MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc15Low)
            {
                _mainViewModel.icc15Low = false;
                _mainViewModel.icc15MessageData = (byte)(_mainViewModel.icc15MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc15Med)
            {
                _mainViewModel.icc15MessageData = (byte)(_mainViewModel.icc15MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc15Med = false;
            }
            if (_mainViewModel.icc15High)
            {
                _mainViewModel.icc15MessageData = (byte)(_mainViewModel.icc15MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc15High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc15On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc15On = true;
            _mainViewModel.Icc15SideMenu = "";
            _mainViewModel.Icc15BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc15BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc15PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc15On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc15Rem)
            {
                _mainViewModel.icc15Rem = true;
    ...
    {
        if (_mainViewModel.icc15On && !_mainViewModel.icc15Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc15Med)
            {
                _mainViewModel.icc15MessageData = (byte)(_mainViewModel.icc15MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc15Low = true;
            _mainViewModel.Icc15SideMenu = "LOW";
            _mainViewModel.Icc15BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc15BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc15PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc15On && !_mainViewModel.icc15Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc15Low)
            {
                _mainViewModel.icc15MessageData = (byte)(_mainViewModel.icc15MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc15Med = true;
            _mainViewModel.Icc15SideMenu = "MED";
            _mainViewModel.Icc15BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc15BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc15PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc15On && !_mainViewModel.icc15High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc15Low)
            {
                _mainViewModel.icc15MessageData = (byte)(_mainViewModel.icc15MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc15High = true;
            _mainViewModel.Icc15SideMenu = "HIGH";
            _mainViewModel.Icc15BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc15BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc15PgBackground = new SolidColorBrush(Colors.OrangeRed);
