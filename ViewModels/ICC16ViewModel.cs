...
public partial class ICC16ViewModel : ViewModelBase
{
    ...
    public ICC16ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC16: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC16 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC16: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC16: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC16: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC16 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC16: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC16: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC16 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC16: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC16: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC16 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc16)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC16: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC16: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC16 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC16: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc16On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc16On = false;
            _mainViewModel.Icc16SideMenu = "OFF";
            _mainViewModel.Icc16BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc16BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc16PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc16Rem = false;

            _mainViewModel.icc16MessageData = (byte)(_mainViewModel.icc16MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc16MessageData = (byte)(_mainViewModel.icc16MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc16Low)
            {
                _mainViewModel.icc16Low = false;
                _mainViewModel.icc16MessageData = (byte)(_mainViewModel.icc16MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc16Med)
            {
                _mainViewModel.icc16MessageData = (byte)(_mainViewModel.icc16MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc16Med = false;
            }
            if (_mainViewModel.icc16High)
            {
                _mainViewModel.icc16MessageData = (byte)(_mainViewModel.icc16MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc16High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc16On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc16On = true;
            _mainViewModel.Icc16SideMenu = "";
            _mainViewModel.Icc16BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc16BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc16PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc16On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc16Rem)
            {
                _mainViewModel.icc16Rem = true;
    ...
    {
        if (_mainViewModel.icc16On && !_mainViewModel.icc16Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc16Med)
            {
                _mainViewModel.icc16MessageData = (byte)(_mainViewModel.icc16MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc16Low = true;
            _mainViewModel.Icc16SideMenu = "LOW";
            _mainViewModel.Icc16BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc16BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc16PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc16On && !_mainViewModel.icc16Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc16Low)
            {
                _mainViewModel.icc16MessageData = (byte)(_mainViewModel.icc16MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc16Med = true;
            _mainViewModel.Icc16SideMenu = "MED";
            _mainViewModel.Icc16BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc16BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc16PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc16On && !_mainViewModel.icc16High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc16Low)
            {
                _mainViewModel.icc16MessageData = (byte)(_mainViewModel.icc16MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc16High = true;
            _mainViewModel.Icc16SideMenu = "HIGH";
            _mainViewModel.Icc16BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc16BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc16PgBackground = new SolidColorBrush(Colors.OrangeRed);
