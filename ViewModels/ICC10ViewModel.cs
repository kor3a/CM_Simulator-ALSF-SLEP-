...
public partial class ICC10ViewModel : ViewModelBase
{
    ...
    public ICC10ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC10: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC10 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC10: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC10: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC10: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC10 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC10: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC10: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC10 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC10: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC10: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC10 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc10)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC10: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC10: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC10 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC10: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc10On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc10On = false;
            _mainViewModel.Icc10SideMenu = "OFF";
            _mainViewModel.Icc10BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc10BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc10PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc10Rem = false;

            _mainViewModel.icc10MessageData = (byte)(_mainViewModel.icc10MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc10MessageData = (byte)(_mainViewModel.icc10MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc10Low)
            {
                _mainViewModel.icc10Low = false;
                _mainViewModel.icc10MessageData = (byte)(_mainViewModel.icc10MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc10Med)
            {
                _mainViewModel.icc10MessageData = (byte)(_mainViewModel.icc10MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc10Med = false;
            }
            if (_mainViewModel.icc10High)
            {
                _mainViewModel.icc10MessageData = (byte)(_mainViewModel.icc10MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc10High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc10On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc10On = true;
            _mainViewModel.Icc10SideMenu = "";
            _mainViewModel.Icc10BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc10BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc10PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc10On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc10Rem)
            {
                _mainViewModel.icc10Rem = true;
    ...
    {
        if (_mainViewModel.icc10On && !_mainViewModel.icc10Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc10Med)
            {
                _mainViewModel.icc10MessageData = (byte)(_mainViewModel.icc10MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc10Low = true;
            _mainViewModel.Icc10SideMenu = "LOW";
            _mainViewModel.Icc10BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc10BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc10PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc10On && !_mainViewModel.icc10Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc10Low)
            {
                _mainViewModel.icc10MessageData = (byte)(_mainViewModel.icc10MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc10Med = true;
            _mainViewModel.Icc10SideMenu = "MED";
            _mainViewModel.Icc10BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc10BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc10PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc10On && !_mainViewModel.icc10High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc10Low)
            {
                _mainViewModel.icc10MessageData = (byte)(_mainViewModel.icc10MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc10High = true;
            _mainViewModel.Icc10SideMenu = "HIGH";
            _mainViewModel.Icc10BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc10BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc10PgBackground = new SolidColorBrush(Colors.OrangeRed);
