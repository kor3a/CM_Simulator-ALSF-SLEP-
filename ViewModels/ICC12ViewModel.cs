...
public partial class ICC12ViewModel : ViewModelBase
{
    ...
    public ICC12ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC12: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC12 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC12: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC12: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC12: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC12 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC12: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC12: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC12 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC12: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC12: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC12 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc12)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC12: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC12: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC12 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC12: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc12On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc12On = false;
            _mainViewModel.Icc12SideMenu = "OFF";
            _mainViewModel.Icc12BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc12BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc12PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc12Rem = false;

            _mainViewModel.icc12MessageData = (byte)(_mainViewModel.icc12MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc12MessageData = (byte)(_mainViewModel.icc12MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc12Low)
            {
                _mainViewModel.icc12Low = false;
                _mainViewModel.icc12MessageData = (byte)(_mainViewModel.icc12MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc12Med)
            {
                _mainViewModel.icc12MessageData = (byte)(_mainViewModel.icc12MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc12Med = false;
            }
            if (_mainViewModel.icc12High)
            {
                _mainViewModel.icc12MessageData = (byte)(_mainViewModel.icc12MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc12High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc12On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc12On = true;
            _mainViewModel.Icc12SideMenu = "";
            _mainViewModel.Icc12BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc12BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc12PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc12On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc12Rem)
            {
                _mainViewModel.icc12Rem = true;
    ...
    {
        if (_mainViewModel.icc12On && !_mainViewModel.icc12Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc12Med)
            {
                _mainViewModel.icc12MessageData = (byte)(_mainViewModel.icc12MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc12Low = true;
            _mainViewModel.Icc12SideMenu = "LOW";
            _mainViewModel.Icc12BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc12BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc12PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc12On && !_mainViewModel.icc12Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc12Low)
            {
                _mainViewModel.icc12MessageData = (byte)(_mainViewModel.icc12MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc12Med = true;
            _mainViewModel.Icc12SideMenu = "MED";
            _mainViewModel.Icc12BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc12BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc12PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc12On && !_mainViewModel.icc12High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc12Low)
            {
                _mainViewModel.icc12MessageData = (byte)(_mainViewModel.icc12MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc12High = true;
            _mainViewModel.Icc12SideMenu = "HIGH";
            _mainViewModel.Icc12BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc12BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc12PgBackground = new SolidColorBrush(Colors.OrangeRed);
