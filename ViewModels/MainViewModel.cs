...
public partial class MainViewModel : ViewModelBase
{
    ...
    private async Task ProcessMessage(byte[] message)
    {
        try
        {
            ...
            Avalonia.Threading.Dispatcher.UIThread.Post(() =>
            {
                _homePage.AppendLog("Received: " + BitConverter.ToString(message) + Environment.NewLine);
                    ...
                    switch (source)
                    {
                        case byte s when s == icc1:
                            ...
                            if(cmMessageData != icc1MessageData)
                            {
                                ...
                                Avalonia.Threading.Dispatcher.UIThread.Post(() => _homePage.AppendLog(mismatchInfo));
                                SendCmCommand(icc1);
                            }
                            break;
                        case byte s when s == icc2:
