...
public partial class ICC11ViewModel : ViewModelBase
{
    ...
    public ICC11ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC11: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC11 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC11: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC11: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC11: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC11 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC11: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC11: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC11 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC11: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC11: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC11 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc11)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC11: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC11: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC11 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC11: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc11On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc11On = false;
            _mainViewModel.Icc11SideMenu = "OFF";
            _mainViewModel.Icc11BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc11BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc11PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc11Rem = false;

            _mainViewModel.icc11MessageData = (byte)(_mainViewModel.icc11MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc11MessageData = (byte)(_mainViewModel.icc11MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc11Low)
            {
                _mainViewModel.icc11Low = false;
                _mainViewModel.icc11MessageData = (byte)(_mainViewModel.icc11MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc11Med)
            {
                _mainViewModel.icc11MessageData = (byte)(_mainViewModel.icc11MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc11Med = false;
            }
            if (_mainViewModel.icc11High)
            {
                _mainViewModel.icc11MessageData = (byte)(_mainViewModel.icc11MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc11High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc11On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc11On = true;
            _mainViewModel.Icc11SideMenu = "";
            _mainViewModel.Icc11BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc11BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc11PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc11On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc11Rem)
            {
                _mainViewModel.icc11Rem = true;
    ...
    {
        if (_mainViewModel.icc11On && !_mainViewModel.icc11Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc11Med)
            {
                _mainViewModel.icc11MessageData = (byte)(_mainViewModel.icc11MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc11Low = true;
            _mainViewModel.Icc11SideMenu = "LOW";
            _mainViewModel.Icc11BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc11BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc11PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc11On && !_mainViewModel.icc11Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc11Low)
            {
                _mainViewModel.icc11MessageData = (byte)(_mainViewModel.icc11MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc11Med = true;
            _mainViewModel.Icc11SideMenu = "MED";
            _mainViewModel.Icc11BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc11BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc11PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc11On && !_mainViewModel.icc11High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc11Low)
            {
                _mainViewModel.icc11MessageData = (byte)(_mainViewModel.icc11MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc11High = true;
            _mainViewModel.Icc11SideMenu = "HIGH";
            _mainViewModel.Icc11BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc11BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc11PgBackground = new SolidColorBrush(Colors.OrangeRed);
