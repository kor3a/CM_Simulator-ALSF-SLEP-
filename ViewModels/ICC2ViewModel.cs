...
public partial class ICC2ViewModel : ViewModelBase
{
    ...
    public ICC2ViewModel(MainViewModel mainViewModel, HomeViewModel homeViewModel)
    {
    ...
    public void SubmitFlasherMisfires(string text)
    {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC2: FlasherMisfires input is empty.\n");
            return;
        }
        ...
        if (int.TryParse(text, out int misfires))
        {
            if (misfires >= 0) // Adjust validation as needed
            {
                ...
                _homeViewModel.AppendToLog($"ICC2 FlasherMisfires updated to {misfires}.\n");
            ...
            {
                _homeViewModel.AppendToLog("Error ICC2: FlasherMisfires must be non-negative.\n");
                FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
            }
        }
        else
        {
            _homeViewModel.AppendToLog("Error ICC2: Invalid FlasherMisfires input. Please enter a number.\n");
            FlasherMisfireBackground = new SolidColorBrush(Colors.Yellow);
        }
    }
    ...
    public void SubmitStartByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC2: Start Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC2 Start Byte updated to 0x{startByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC2: Invalid Start Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitDestinationByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC2: Destination Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($" ICC2 Destination Byte updated to 0x{destinationByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC2: Invalid Destination Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitSourceByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC2: Source Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC2 Source Byte updated to 0x{sourceByte.ToString("X2")}.\n");

            // Update cmMessageData or other logic if needed
            _mainViewModel.SendCmCommand(); // Send updated cmMessageData

            if (sourceByte != _mainViewModel.icc2)
            {
        ...
        {
            _homeViewModel.AppendToLog("Error ICC2: Invalid Source Byte input. Please enter a byte number without '0x'.\n");
    ...
    public void SubmitEndByte(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
        {
            _homeViewModel.AppendToLog("Error ICC2: End Byte input is empty.\n");
            return;
        }
        ...
        if (isValidInput)
        {
            ...
            _homeViewModel.AppendToLog($"ICC2 End Byte updated to 0x{endByte.ToString("X2")}.\n");
        ...
        {
            _homeViewModel.AppendToLog("Error ICC2: Invalid End Byte input. Please enter a byte number without '0x'.\n");
    ...
    {
        if (_mainViewModel.icc2On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc2On = false;
            _mainViewModel.Icc2SideMenu = "OFF";
            _mainViewModel.Icc2BorderBrush = new SolidColorBrush(Colors.Black);
            _mainViewModel.Icc2BorderBackground = new SolidColorBrush(Colors.Black);
            _homeViewModel.Lvicc2PgBackground = new SolidColorBrush(Colors.LightGray);
            ...
            _mainViewModel.icc2Rem = false;

            _mainViewModel.icc2MessageData = (byte)(_mainViewModel.icc2MessageData ^ _mainViewModel.fOffByte);
            _mainViewModel.icc2MessageData = (byte)(_mainViewModel.icc2MessageData ^ _mainViewModel.fOnByte);

            if (_mainViewModel.icc2Low)
            {
                _mainViewModel.icc2Low = false;
                _mainViewModel.icc2MessageData = (byte)(_mainViewModel.icc2MessageData ^ _mainViewModel.fLowByte);
            }
            if (_mainViewModel.icc2Med)
            {
                _mainViewModel.icc2MessageData = (byte)(_mainViewModel.icc2MessageData ^ _mainViewModel.fMedByte);
                _mainViewModel.icc2Med = false;
            }
            if (_mainViewModel.icc2High)
            {
                _mainViewModel.icc2MessageData = (byte)(_mainViewModel.icc2MessageData ^ _mainViewModel.fHighByte);
                _mainViewModel.icc2High = false;
            }
    ...
    public void OnButtonClicked()
    {
        if (!_mainViewModel.icc2On && !_mainViewModel.cmAuto)
        {
            _mainViewModel.icc2On = true;
            _mainViewModel.Icc2SideMenu = "";
            _mainViewModel.Icc2BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc2BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc2PgBackground = new SolidColorBrush(Colors.Green);
    ...
    public void RemButtonClicked()
    {
        if (_mainViewModel.icc2On && !_mainViewModel.cmAuto)
        {
            if (!_mainViewModel.icc2Rem)
            {
                _mainViewModel.icc2Rem = true;
    ...
    {
        if (_mainViewModel.icc2On && !_mainViewModel.icc2Low && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc2Med)
            {
                _mainViewModel.icc2MessageData = (byte)(_mainViewModel.icc2MessageData ^ _mainViewModel.fMedByte);
            ...
            _mainViewModel.icc2Low = true;
            _mainViewModel.Icc2SideMenu = "LOW";
            _mainViewModel.Icc2BorderBrush = new SolidColorBrush(Colors.Green);
            _mainViewModel.Icc2BorderBackground = new SolidColorBrush(Colors.Green);
            _homeViewModel.Lvicc2PgBackground = new SolidColorBrush(Colors.Green);
    ...
    {
        if (_mainViewModel.icc2On && !_mainViewModel.icc2Med && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc2Low)
            {
                _mainViewModel.icc2MessageData = (byte)(_mainViewModel.icc2MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc2Med = true;
            _mainViewModel.Icc2SideMenu = "MED";
            _mainViewModel.Icc2BorderBrush = new SolidColorBrush(Colors.Orange);
            _mainViewModel.Icc2BorderBackground = new SolidColorBrush(Colors.Orange);
            _homeViewModel.Lvicc2PgBackground = new SolidColorBrush(Colors.Orange);
    ...
    {
        if (_mainViewModel.icc2On && !_mainViewModel.icc2High && !_mainViewModel.cmAuto)
        {
            if (_mainViewModel.icc2Low)
            {
                _mainViewModel.icc2MessageData = (byte)(_mainViewModel.icc2MessageData ^ _mainViewModel.fLowByte);
            ...
            _mainViewModel.icc2High = true;
            _mainViewModel.Icc2SideMenu = "HIGH";
            _mainViewModel.Icc2BorderBrush = new SolidColorBrush(Colors.OrangeRed);
            _mainViewModel.Icc2BorderBackground = new SolidColorBrush(Colors.OrangeRed);
            _homeViewModel.Lvicc2PgBackground = new SolidColorBrush(Colors.OrangeRed);
